...
public class ObjectiveManager : MonoBehaviour
{
    ...
    private IEnumerator AnimatePanel(bool show)
    {
        ...
        if (show) objectiveText.gameObject.SetActive(true);

        float startAlpha = _canvasGroup.alpha;
        float targetAlpha = show ? 1f : 0f;

        Vector2 hiddenPos = _originalAnchoredPos;
        if (slideHorizontal) hiddenPos.x -= slideDistance; // Slide left when hidden
        else hiddenPos.y += slideDistance; // Slide up when hidden
        ...
        // Disable object entirely when finished hiding to save performance
        if (!show) objectiveText.gameObject.SetActive(false);
    }
